...
output_T_2 = Conv1D(filters=6 , kernel_size = 5, strides=2, activation='relu', padding='same')(output_T_1)

#output_T_2 = MaxPooling1D(pool_size=5, padding='same')(output_T_2)

output_T_3 = Conv1D(filters=36 , kernel_size = 10, strides=4 ,activation='relu', padding='same')(output_T_2)

#output_T_3 = MaxPooling1D(pool_size=5, padding='same')(output_T_3)

output_T_4 = Conv1D(filters=72 , kernel_size = 20, strides=4,activation='relu', padding='same')(output_T_3)

#output_T_4 = MaxPooling1D(pool_size=4, padding='same')(output_T_4)

output_T_5 = Conv1D(filters=144 , kernel_size = 40, strides=4, activation='relu', padding='same')(output_T_4)

#output_T_5 = MaxPooling1D(pool_size=3, padding='same')(output_T_5)

Encoded = Conv1D(filters=144 , kernel_size = 40,strides= 5, activation='relu', padding='same')(output_T_5)

decoded1 = Conv1D(filters=144 , kernel_size = 40,activation='relu', padding='same')(Encoded)

decoded1 = UpSampling1D(size=5)(decoded1)

decoded1 = Conv1D(filters=144 , kernel_size = 40,activation='relu', padding='same')(decoded1)

decoded1 = UpSampling1D(size=4)(decoded1)


decoded1 = Conv1D(filters=72 , kernel_size = 20,activation='relu', padding='same')(decoded1)

decoded1 = UpSampling1D(size=4)(decoded1)

decoded1 = Conv1D(filters=36 , kernel_size = 10,activation='relu', padding='same')(decoded1)

decoded1 = UpSampling1D(size=4)(decoded1)

decoded1 = Conv1D(filters=6 , kernel_size = 5,activation='relu', padding='same')(decoded1)

decoded1 = UpSampling1D(size=2)(decoded1)

decoded = Conv1D(filters=1, kernel_size=5, activation = 'tanh', padding='same')(decoded1) # Values between -1 and 1 thus tanh
...
callbacks = [
	...
	ModelCheckpoint('weights/weight_autoenc_5L_10_truediff2.0.hdf5', monitor='val_loss', verbose=0, save_best_only='true', save_weights_only='true', mode='auto', period=1)
]
...
Autoencodeur.save('models/Auto_encoder_5L_10_truediff2.0.h5')
