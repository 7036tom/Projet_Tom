...
with open("databases/dbwhole_test.csv", "wb") as db:
	writer = csv.writer(db)
	for i in range(20000,Taille_sousB): # taillesousb                                                                                                                                                            
		try:
			X, sample_rate = librosa.load("raw_magnatagatune_data/"+Path[i][0], res_type='kaiser_fast', sr = 16000)#, sr = 16000)  # Probably an error due to try : if someting fails X stays empty at the line of the failed event.                        
			X[len(X)-1]=i 
			writer.writerow(X)
		except:
